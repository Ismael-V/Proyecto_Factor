...
void generateSemiprimes(FILE* salida, uint16_t bits_p, uint16_t bits_q, uint16_t cantidad){
...
int main(int argc, char** argv){
    ...
    if(argc != 5){
        cout << "benchmark_gen <fichero.txt> <bits_p> <bits_q> <cantidad>\n";
        return 1;
    }

    bits_p = atoi(argv[2]);
    bits_q = atoi(argv[3]);
    cantidad = atoi(argv[4]);

    FILE* descSalida = fopen(argv[1], "w");
    if(descSalida != 0){
        cout << "Generando bechmarks . . .\n";
        generateSemiprimes(descSalida, bits_p, bits_q, cantidad);
        fclose(descSalida);
