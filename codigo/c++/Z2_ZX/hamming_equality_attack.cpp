...
void solve_factor(std::vector<Z2_poly<U_TYPE>>& factores, uint32_t grado_target, const mpz_t N, mpz_t& factor){
    ...
    std::vector<Z2_poly<U_TYPE>>::const_iterator* factor_apuntado = new std::vector<Z2_poly<U_TYPE>>::const_iterator[num_factores];
    ...
    while(anidamiento >= 0){
        ...
        if(factor_apuntado[anidamiento] != factores.end()){
...
void test_equality_attack(mpz_t &clave_publica, mpz_t &p){
    ...
    for(uint32_t i = 0; i < 7; i++){
        ...
        //Buscamos la clave
        solve_factor(factores, grados_target[i], clave_publica, p);
